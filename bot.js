...
async function startBot(number){
  ...
  sock.ev.on("messages.upsert", async m=>{
    for(const msg of m.messages){
      ...
      else if(lower==="2") await sock.sendMessage(from,{text:`*2 FRIEND* 😊 [${number}] Yo!`})
      else if(lower==="3") await sock.sendMessage(from,{text:`*3 FAMILY* 👨‍👩‍👧‍👦 [${number}] Hello family`})
      else if(lower==="4") await sock.sendMessage(from,{text:`*4 WORKSHOP* 🔧 [${number}] 0746622284`})
      else if(lower==="5") await sock.sendMessage(from,{text:`*5 ORDER* 📦 [${number}]\nName:\nItem:\nSize:\nLocation:`})
      else if(lower==="6") await sock.sendMessage(from,{text:`*6 READY* 🚪 [${number}] Ready doors!`})
      else if(lower==="7") await sock.sendMessage(from,{text:`*7 GREETINGS* 👋 [${number}] Thanks!`})
      else if(lower==="8") await sock.sendMessage(from,{text:`*8 JOKE* 🤣 Why welder broke up? Too many sparks!`})
      else if(lower==="9") await sock.sendMessage(from,{text:`*9 UPDATE* 📢 [${number}] Send update`})
      else if(lower==="10") await sock.sendMessage(from,{text:`*10 FIRST TIME* 🆕 [${number}] Welcome! 0746622284`})
...
app.get("/", checkAdmin, (req,res)=>{
  ...
  res.send(`<html><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{background:#0f1115;color:#fff;font-family:sans-serif;display:flex;justify-content:center;padding:20px;margin:0}.card{background:#1a1d24;padding:20px;border-radius:15px;width:100%;max-width:460px;text-align:center}input{width:100%;padding:14px;border-radius:10px;border:1px solid #333;background:#0f1115;color:#fff;box-sizing:border-box}button{width:100%;padding:13px;border-radius:10px;border:0;background:#22c55e;color:#000;font-weight:bold;margin-top:10px;cursor:pointer}.codeBox{border:2px dashed #22c55e;padding:18px;margin:12px 0;border-radius:12px;position:relative;background:#0f1115}.codeTxt{font-size:30px;color:#22c55e;font-weight:bold;letter-spacing:4px}.numCard{background:#0f1115;border:1px solid #333;padding:10px;border-radius:10px;margin:6px 0;display:flex;justify-content:space-between;font-size:13px}.online{color:#22c55e}</style><script>function copyC(){const t=document.getElementById('c').innerText;navigator.clipboard.writeText(t);document.getElementById('b').innerText='COPIED ✓';let i=60;let x=setInterval(()=>{document.getElementById('timer').innerText='Expires in '+i+'s';i--;if(i<0){clearInterval(x);document.getElementById('timer').innerText='CODE EXPIRED - Get new one';}},1000)}window.onload=()=>{if(document.getElementById('c')) copyC()}</script><body><div class="card"><h2>👑 MCREZIL MULTI</h2><div style="font-size:11px;color:#888">Key:22669988 | Saved:${folders.length} | Online:${active.length}</div><div style="font-size:10px;color:#22c55e;margin:6px 0">${CHANNEL_LINK}</div>${lastCode?`<div class="codeBox"><div style="font-size:11px;color:#aaa">POPUP SENT TO ${escapeHtml(lastNum)} 🔔</div><div id="c" class="codeTxt">${escapeHtml(lastCode)}</div><div id="timer" style="font-size:12px;color:#ffcc00;margin:6px">Expires in 60s</div><div style="font-size:11px;color:#888;margin-top:8px">1. Open WhatsApp on <b>${escapeHtml(lastNum)}</b><br>2. Settings > Linked Devices<br>3. Link with phone number<br>4. Paste code above ☝️</div><button id="b" onclick="copyC()" style="width:auto;padding:6px 14px;background:#fff;color:#000;margin-top:10px">COPY CODE</button></div>`:`<div style="color:#666;margin:12px;font-size:12px">No code yet - Add ONE number below</div>`}<form method="POST" action="/pair?key=${escapeHtml(key)}"><input name="number" placeholder="256746622284 (ONE number)" required><button>GET CODE + SEND POPUP 🔔</button></form><h3 style="text-align:left;font-size:12px;margin-top:16px">📱 BOT NUMBERS (MULTI HOLD):</h3>${folders.map(f=>{const on=active.includes(f);return `<div class="numCard"><span>${escapeHtml(f)} <span class="${on?'online':''}">${on?'● ONLINE':'○ OFFLINE'}</span></span><form method="POST" action="/delete?key=${escapeHtml(key)}" style="margin:0;width:auto"><input type="hidden" name="number" value="${escapeHtml(f)}"><button style="width:auto;padding:5px 10px;background:#ff4444;color:#fff">DEL</button></form></div>`}).join("")}</div></body></html>`)
})
...
app.post("/pair", checkAdmin, async (req,res)=>{
  ...
  try{
    ...
    await delay(2500)
    // THIS TRIGGERS WHATSAPP POPUP NOTIFICATION 🔔
    const code = await clients[num].requestPairingCode(num)
    codes[num]=code
    console.log(`🔔 POPUP -> ${num} CODE ${code}`)
...
app.listen(PORT, '0.0.0.0', ()=>console.log(`MCREZIL RUNNING on ${PORT} - POPUP READY`))
